...
def authenticated_menu():
    ...
    chatbot_label = config["company_name"] + " ChatBot"
    ...
    st.sidebar.markdown(f"[{config['contact_label']}]({config['contact_page']})", unsafe_allow_html=True)
    st.sidebar.markdown(f"[Powered by AIFAQ]({config['landing_page']})", unsafe_allow_html=True)
...
def unauthenticated_menu():
    ...
    if not demo:
        ...
        st.sidebar.info("Demo mode: Login disabled.")

    st.sidebar.markdown(f"[Back to {config['company_name']}]({config['client_page']})", unsafe_allow_html=True)
    st.sidebar.markdown(f"[{config['contact_label']}]({config['contact_page']})", unsafe_allow_html=True)
    st.sidebar.markdown(f"[Powered by AIFAQ]({config['landing_page']})", unsafe_allow_html=True)
    st.sidebar.page_link("app.py", label="About us")
