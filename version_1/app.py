...
for k in ("sessions", "current", "llm", "last_upload_time", "kb_refresh"):
    st.session_state.setdefault(k,
        {}             if k == "sessions" else
        LLM_OPTIONS[0] if k == "llm" else
        0              if k == "last_upload_time" else
        False          if k == "kb_refresh" else
        None)
...
if not st.session_state.current:
    ...
    st.session_state.sessions[st.session_state.current] = {"title": "New chat", "messages": []}
...
def extract_pptx_full(f):
    ...
    try:
        ...
        for slide in prs.slides:
            slide_text = " ".join(shape.text for shape in slide.shapes if hasattr(shape, "text") and shape.text.strip())
            if slide_text:
...
def chunk_text(txt: str, size: int = MAX_CHUNK_SIZE, overlap: int = CHUNK_OVERLAP) -> list[str]:
...
def store_document_fast(filename: str, src_type: str, content: str, public: bool = False) -> bool:
    """
    ...
    try:
        ...
        session.sql(
            ...
            f"{public},CURRENT_TIMESTAMP())").collect()
        ...
        if chunk_data:
            ...
            verify = session.sql(f"SELECT COUNT(*) as cnt FROM {DOCS_TBL} WHERE DOC_ID='{sanitize(doc_id)}'").collect()
            if verify and verify[0][0] > 0:
                return True
            else:
                st.error(f"❌ {filename}: Verification failed - document not found in database")
                return False
        ...
        try:
            session.sql(f"DELETE FROM {EMB_TBL} WHERE CHUNK_ID IN (SELECT CHUNK_ID FROM {CHUNKS_TBL} WHERE DOC_ID='{sanitize(doc_id)}')").collect()
            session.sql(f"DELETE FROM {CHUNKS_TBL} WHERE DOC_ID='{sanitize(doc_id)}'").collect()
            session.sql(f"DELETE FROM {DOCS_TBL} WHERE DOC_ID='{sanitize(doc_id)}'").collect()
...
with st.sidebar:
    ...
    if files and st.button("⚡ Upload & Process", use_container_width=True, type="primary"):
        ...
        if df.empty:
            ...
            for _, r in df.iterrows():
                ...
                with st.expander(f"{icon} {r['FILENAME']}", expanded=False):
                    ...
                    st.caption(f"**Uploaded:** {r['UPLOADED_AT'].strftime('%Y-%m-%d %H:%M')}")
                    
                    if st.button("🗑️ Delete", key=f"del_{r['DOC_ID']}", use_container_width=True):
    ...
    try:
        ...
        if not chats.empty:
            for _, c in chats.iterrows():
                ...
                if st.button(f"💬 {btn_label}", key=f"chat_{c['SESSION_ID']}", use_container_width=True):
    ...
    if st.button("🗑️ Clear current chat", use_container_width=True):
        try:
            session.sql(f"DELETE FROM {CHAT_TBL} WHERE SESSION_ID='{sanitize(st.session_state.current)}'").collect()
            ...
            try:
                ...
                if rows.empty:
                    ...
                    for _, row in rows.iterrows():
                        context_parts.append(f"[From {row['FILENAME']} ({row['FILE_TYPE']})]\n{row['CHUNK_TEXT']}")
                    context = "\n\n---\n\n".join(context_parts)
                    ...
                    answer = str(ans_rows[0][0]) if ans_rows else "The LLM service didn't return a response."
                    
                    # Cite only the PRIMARY source (highest similarity score)
                    # Group by filename and get the highest similarity for each document
                    source_scores = rows.groupby('FILENAME')['SIMILARITY_SCORE'].max().sort_values(ascending=False)
        ...
        try:
            session.write_pandas(
                pd.DataFrame([{
                    ...
                    "QUERY_TIMESTAMP": dt.datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')
                }]),
                table_name=CHAT_TBL, 
                auto_create_table=False, 
                overwrite=False
